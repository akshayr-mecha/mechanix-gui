...
fn insert_test_schema(db: &mut Database, schema_name: &str) {
    ...
    db.insert_checksum(schema_name, "checksums", &1234)
        .expect("Failed to insert schema checksum");

    // Insert schema metadata
    let schema_id = schema_name.replace(".toml", "");

    // Insert test keys with their descriptions and default values
    db.insert_settings(&schema_id, "section.key", b"A test key")
        .expect("Failed to insert key");
    db.insert_settings(&schema_id, "section.bool_key", b"A boolean key")
        .expect("Failed to insert bool key");
    db.insert_settings(&schema_id, "section.number_key", b"A number key")
        .expect("Failed to insert number key");
    db.insert_settings(&schema_id, "section.enum_key", b"An enum key")
...
async fn test_list_schemas_with_data() {
    ...
    let schemas_json = result.unwrap();
    assert!(
        schemas_json.contains("org.mechanix.test.toml"),
        "Schemas list should contain the test schema"
    );
}
...
async fn test_list_keys() {
    ...
    assert!(
        keys.contains(&"section.key".to_string()),
        "Keys list should contain section.key"
    );
    assert!(
        keys.contains(&"section.bool_key".to_string()),
        "Keys list should contain section.bool_key"
    );
    assert!(
        keys.contains(&"section.number_key".to_string()),
        "Keys list should contain section.number_key"
    );
    assert!(
        keys.contains(&"section.enum_key".to_string()),
        "Keys list should contain section.enum_key"
    );
}
...
async fn test_list_keys_nonexistent_schema() {
    ...
    let result = interface
        .list_keys("org.mechanix.nonexistent".to_string())
        .await;
    assert!(
        result.is_err(),
        "List keys should fail for nonexistent schema"
    );
}
...
async fn test_describe_key() {
    ...
    // Describe a key
    let result = interface
        .describe_key("org.mechanix.test".to_string(), "section.key".to_string())
        .await;
...
async fn test_describe_key_nonexistent() {
    ...
    assert!(
        result.is_err(),
        "Describe key should fail for nonexistent key"
    );
}
...
async fn test_get_setting_nonexistent() {
    ...
    assert!(
        result.is_ok(),
        "Get setting should succeed even for nonexistent settings"
    );

    let value = result.unwrap();
    assert!(
        value.is_empty(),
        "Value should be empty for nonexistent setting"
    );
}
...
async fn test_set_and_get_setting() {
    ...
    // Set a setting
    let result = interface
        .set_setting("org.mechanix.test.section.key", "test_value")
        .await;
    assert!(result.is_ok(), "Set setting should succeed");
    assert_eq!(
        result.unwrap(),
        "Success",
        "Set setting should return 'Success'"
    );
    ...
    let value = result.unwrap();
    assert!(
        value.contains_key("org.mechanix.test.section.key"),
        "Result should contain the key"
    );
    assert_eq!(
        value.get("org.mechanix.test.section.key").unwrap(),
        "test_value",
...
async fn test_set_setting_nonexistent_schema() {
    ...
    let result = interface
        .set_setting("org.mechanix.nonexistent.section.key", "test_value")
        .await;
    assert!(
        result.is_err(),
        "Set setting should fail for nonexistent schema"
    );
    let err = result.unwrap_err();
    assert!(
        err.to_string().contains("Failed to read schema file"),
        "Error should contain 'Failed to read schema file'"
    );
}
