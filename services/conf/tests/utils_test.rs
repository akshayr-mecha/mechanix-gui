...
"#;
    ...
    assert!(
        result.is_ok(),
        "Failed to read application schema: {:?}",
        result.err()
    );
...
fn test_read_application_schema_nonexistent_file() {
    ...
    assert!(
        result.is_err(),
        "Expected error when reading nonexistent file"
    );
    ...
    assert!(
        err_string.contains("Unable to open file"),
        "Error message does not contain expected text: {}",
        err_string
    );
    assert!(
        err_string.contains("nonexistent_file.toml"),
        "Error message does not contain the file name: {}",
        err_string
    );
}
